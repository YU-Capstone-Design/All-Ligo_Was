...
public interface ContentAPI {
    ...
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "생성 완료 콘텐츠 미리보기 조회 성공"
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "생성 완료 상태가 아닌 콘텐츠를 조회한 경우",
                    content = @Content(
                            mediaType = "application/json",
    ...
    ResponseEntity<ContentPreviewResDto> getContentPreview(
            @Parameter(description = "미리보기할 콘텐츠 ID", example = "1")
            @PathVariable Long contentId
    );

    @Operation(
            summary = "생성 콘텐츠 배포 중단",
            description = """
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "400",
                    description = "배포 중단할 수 없는 콘텐츠 상태",
                    content = @Content(
                            mediaType = "application/json",
                            examples = {
                                    @ExampleObject(
    ...
    ResponseEntity<ContentCancelResDto> cancelContent(
            @Parameter(description = "배포 중단할 콘텐츠 ID", example = "1")
            @PathVariable Long contentId
    );
}
